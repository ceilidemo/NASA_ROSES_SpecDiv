...
ggsave("Modeling/figures/significant_topdown_pairs.jpg", sig_topdown_plot, width = 13, height = 10, dpi = 300)
...
taxonomic_table <- all_pairs_table %>% 
  filter(Plant_Metric %in% topdown_taxonomic)

phylogenetic_table <- all_pairs_table %>% 
  filter(Plant_Metric %in% topdown_phylogenetic)

functional_table <- all_pairs_table %>% 
  filter(Plant_Metric %in% topdown_functional)

write.csv(taxonomic_table, "Modeling/data_out/topDown_taxonomic_correlations.csv", row.names = FALSE)
write.csv(phylogenetic_table, "Modeling/data_out/topDown_phylogenetic_correlations.csv", row.names = FALSE)
write.csv(functional_table, "Modeling/data_out/topDown_functional_correlations.csv", row.names = FALSE)
